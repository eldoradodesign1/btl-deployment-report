...
import { createPersistentLead, deletePersistentLead, updatePersistentLead } from "@/lib/supabaseActions";
import { getOrGenerateDailyComment, getOrGenerateWeeklyComment, regenerateDailyComment, regenerateWeeklyComment, saveDailyComment, saveWeeklyComment } from "@/lib/supervisorComments";
...
// Design philosophy: Halo Opaline retains a stable pilotage layout; preference persistence is additive and invisible to the interface.
...
const readTargetSwitchPreference = (key: string) => { try { return localStorage.getItem(key) === "true"; } catch { return false; } };
const writeTargetSwitchPreference = (key: string, value: boolean) => { try { localStorage.setItem(key, String(value)); } catch { /* The toggle remains usable if browser storage is unavailable. */ } return value; };
...
export default function Home() {
  ...
  const onSupabaseSync = async (config: SupabaseConfig) => { const snapshot = await loadSupabaseSnapshot(createSupabaseClient(config)); setData(snapshot.records); setDailyComments(snapshot.dailyComments); setWeeklyComments(snapshot.weeklyComments); setDataSource("supabase"); saveSupabaseConfig(config); setSupabaseConfig(config); const nextDates = dateList(snapshot.records); setStartDate(nextDates[0] ?? "2026-07-23"); setEndDate(nextDates.at(-1) ?? "2026-08-08"); };
  ...
  const onEditRecord = async (index: number, field: "a" | "s" | "cl" | "n", value: string) => { const current = data[index]; if (!current) return; await updatePersistentLead(current, field, value); setData((rows) => rows.map((item, itemIndex) => itemIndex === index ? { ...item, [field]: value } : item)); };
  const onCreateActivation = async (record: Activation) => { const persisted = await createPersistentLead(record); setData((current) => [persisted, ...current]); };
  const onDeleteActivation = async (record: Activation) => { await deletePersistentLead(record); setData((current) => current.filter((item) => item.id !== record.id)); };
  const saveComment = async (detail: CommentDetail, comment: string) => { if (detail.source === "weekly") { await saveWeeklyComment(detail.week ?? 1, detail.date, detail.endDate ?? detail.date, comment, data); setWeeklyComments((current) => ({ ...current, [`${detail.date}|${detail.endDate ?? detail.date}`]: comment })); } else { await saveDailyComment(detail.date, comment, data); setDailyComments((current) => ({ ...current, [detail.date]: comment })); } setCommentDetail((current) => current ? { ...current, comment } : current); };
  const generateComment = async (detail: CommentDetail) => { if (detail.source === "weekly") { const comment = detail.comment.trim() ? await regenerateWeeklyComment(detail.week ?? 1, detail.date, detail.endDate ?? detail.date, data) : await getOrGenerateWeeklyComment(detail.week ?? 1, detail.date, detail.endDate ?? detail.date, data); setWeeklyComments((current) => ({ ...current, [`${detail.date}|${detail.endDate ?? detail.date}`]: comment })); return comment; } const comment = detail.comment.trim() ? await regenerateDailyComment(detail.date, data) : await getOrGenerateDailyComment(detail.date, data); setDailyComments((current) => ({ ...current, [detail.date]: comment })); return comment; };
  const openDailyComment = async (date: string, value: number, source: "curve" | "histogram") => {
    ...
    try {
      const comment = await getOrGenerateDailyComment(date, data);
