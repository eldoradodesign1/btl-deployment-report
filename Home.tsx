...
function TargetOverlay({ values, max }: { values: number[]; max: number }) { if (!values.length) return null; const width = 1000; const height = 100; const points = values.map((value, index) => `${(index / Math.max(1, values.length - 1)) * width},${height - 3 - (value / Math.max(max, 1)) * (height - 8)}`).join(" "); return <svg className="bar-target-overlay" viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none" aria-hidden="true"><polyline points={points} /></svg>; }
...
function DashboardContent({ metrics, filtered, daily, targetDaily, fullDaily, fullTargetDaily, dailyComments, rangeLabel, startDate, endDate, role, onRangeChange, onToggleProgressTargets, onToggleHistogramTargets }: { metrics: ReturnType<typeof metricsOf>; filtered: Activation[]; daily: [string, number][]; targetDaily: [string, number][]; fullDaily: [string, number][]; fullTargetDaily: [string, number][]; dailyComments: Record<string, string>; rangeLabel: string; startDate: string; endDate: string; role: Role; onRangeChange: (startDate: string, endDate: string) => void; onToggleProgressTargets?: () => void; onToggleHistogramTargets?: () => void }) {
  const [commentDetail, setCommentDetail] = useState<CommentDetail | null>(null);
  const histogramTargetsVisible = targetDaily.length > 0;
  const targetByDate = new Map(targetDaily); const targetValues = daily.map(([date]) => targetByDate.get(date) ?? 0); const maxDay = Math.max(...daily.map(([, value]) => value), ...targetValues, 1); const bestDay = daily.reduce<[string, number] | null>((best, current) => !best || current[1] > best[1] ? current : best, null);
