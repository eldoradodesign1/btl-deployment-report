...
import { authenticateSupabaseAdmin, createSupabaseClient, logBtlDataChange, readSupabaseConfig, saveSupabaseConfig, updateBtlProfile, type SupabaseAdminProfile, type SupabaseBtlProfile, type SupabaseConfig } from "@/lib/supabase";
...
export default function Home() {
  ...
  const auditDataChange = async (action: string, entityId: string | undefined, summary: string, beforeState?: Record<string, unknown>, afterState?: Record<string, unknown>) => { if (!session?.profile) return; await logBtlDataChange({ actor_user_id: session.profile.id, actor_name: session.profile.full_name, actor_role: session.profile.role, action, entity_type: "lead", entity_id: entityId ?? null, campaign_id: selectedCampaign?.id ?? null, summary, before_state: beforeState ?? null, after_state: afterState ?? null }); };
