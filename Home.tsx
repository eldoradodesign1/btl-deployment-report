...
import { readGeminiBrowserConfig } from "@/lib/gemini";
...
export default function Home() {
  ...
  const onEditRecord = async (index: number, field: "a" | "s" | "cl" | "n", value: string) => { const current = data[index]; if (!current) return; await updatePersistentLead(current, field, value); setData((rows) => { const next = rows.map((item, itemIndex) => itemIndex === index ? { ...item, [field]: value } : item); saveLocalData(next); return next; }); };
  const onCreateActivation = async (record: Activation) => { const persisted = await createPersistentLead(record); setData((current) => { const next = [persisted, ...current]; saveLocalData(next); return next; }); };
  const onDeleteActivation = async (record: Activation) => { await deletePersistentLead(record); setData((current) => { const next = current.filter((item) => item.id !== record.id); saveLocalData(next); return next; }); };
  ...
  const openDailyComment = async (date: string, value: number, source: "curve" | "histogram") => {
    ...
    if (session?.role !== "btl" || detail.comment.trim() || !readGeminiBrowserConfig().apiKey) return;
