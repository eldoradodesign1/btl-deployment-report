...
import { localPeriodCommentKey, readLocalPeriodComments, writeLocalPeriodComments } from "@/lib/localPeriodComments";
...
export default function Home() {
  ...
  const weeklyComment = exactWeek ? weeklyComments[`${startDate}|${endDate}`] ?? "" : "";
  ...
  const onLogin = (role: Role) => { setTargetSettingsLoading(true); const next = writeSession(role); setSession(next); };
  const onSwitchRole = (role: Role) => { if (role === "btl" && session?.role === "vodacom") { setAccountPasswordOpen(true); setAccountPassword(""); setAccountPasswordError(""); return; } setTargetSettingsLoading(true); const next = writeSession(role); setSession(next); setPage("dashboard"); setAccountOpen(false); setAccountPasswordOpen(false); };
  const confirmBtlSwitch = (event: React.FormEvent) => { event.preventDefault(); if (accountPassword !== "BTL2026") { setAccountPasswordError("Mot de passe incorrect."); return; } setTargetSettingsLoading(true); const next = writeSession("btl"); setSession(next); setPage("dashboard"); setAccountOpen(false); setAccountPasswordOpen(false); setAccountPassword(""); setAccountPasswordError(""); };
  const onLogout = () => { localStorage.removeItem("vodacom-session"); setTargetSettingsLoading(false); setSession(null); setPage("dashboard"); setAccountOpen(false); setAccountPasswordOpen(false); setAccountPassword(""); setAccountPasswordError(""); setHelpOpen(false); setHelpGuideOpen(false); };
  useEffect(() => {
  ...
  const saveComment = async (detail: CommentDetail, comment: string) => { if (detail.source === "custom") { const nextComment = comment.trim(); setCustomPeriodComments((current) => { const next = { ...current, [customPeriodKey]: nextComment }; writeLocalPeriodComments(next); return next; }); } else if (detail.source === "weekly") { await saveWeeklyComment(detail.week ?? 1, detail.date, detail.endDate ?? detail.date, comment, data); setWeeklyComments((current) => ({ ...current, [`${detail.date}|${detail.endDate ?? detail.date}`]: comment })); } else { await saveDailyComment(detail.date, comment, data); setDailyComments((current) => ({ ...current, [detail.date]: comment })); } setCommentDetail((current) => current ? { ...current, comment } : current); };
...
function DashboardContent({ metrics, filtered, daily, targetDaily, fullDaily, fullTargetDaily, dailyComments, rangeLabel, startDate, endDate, role, targetsLoading = false, onCommentOpen, onRangeChange, onToggleProgressTargets, onToggleHistogramTargets, showProgressTargets = false, showHistogramTargets = false }: { metrics: ReturnType<typeof metricsOf>; filtered: Activation[]; daily: [string, number][]; targetDaily: [string, number][]; fullDaily: [string, number][]; fullTargetDaily: [string, number][]; dailyComments: Record<string, string>; rangeLabel: string; startDate: string; endDate: string; role: Role; targetsLoading?: boolean; onCommentOpen: (date: string, value: number, source: "curve" | "histogram") => void; onRangeChange: (startDate: string, endDate: string) => void; onToggleProgressTargets?: () => void; onToggleHistogramTargets?: () => void; showProgressTargets?: boolean; showHistogramTargets?: boolean }) {
